  ...
  return (
    <div className="flex w-full flex-col items-center justify-between gap-y-3 md:flex-row md:gap-y-0">
      <nav className="flex h-[3rem] w-full flex-row items-center justify-center gap-x-1 md:justify-start">
        {isMediumScreen && (
          <div className="hidden items-center gap-x-5 rounded-full bg-neutral-200 p-1 md:flex">
            <h3 className="pl-3 text-xs font-semibold">{myEmail}</h3>
            <button
              className="custom-button-white w-[5rem] px-5 py-3.5 text-xs"
        ...
        {!isMediumScreen && (
          <Link href={`mailto:${myEmail}`} target="_blank" passHref>
            <button
              className="custom-button-black px-5 py-3.5 text-xs md:hidden"
              aria-label="Copy Email"
              onClick={() => handleCopyEmail(myEmail)}
            >
              Email
        ...
        {isMediumScreen && (
          <div className="hidden rounded-full bg-neutral-200 p-1 md:flex">
            <Link
              href={myResume}
              target="_blank"
              className="custom-button-white px-5 py-3.5 text-xs"
        ...
        {!isMediumScreen && (
          <div className="flex rounded-full bg-neutral-200 p-1 md:hidden">
            <Link
              href={myResume}
              target="_blank"
              aria-label="Download Resume"
